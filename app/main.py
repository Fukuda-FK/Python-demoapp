...
async def lifespan(app: FastAPI):
    ...
    await init_database()
    infra_logger.info('[INFRA] Application startup completed')
    print("========================================")
    print("  FastAPI ワークロード デモシステム")
    print("========================================")
    yield
    infra_logger.info('[INFRA] Application shutdown initiated')
    await db_pool.close()
...
async def payment(req: PaymentRequest):
    ...
    if code_error_mode:
        ...
        infra_logger.error(f'[INFRA] Application error detected: ZeroDivisionError, Amount: ¥{req.amount}, Store: {req.storeId}')
    ...
    if db_error_mode:
        ...
        infra_logger.error(f'[INFRA] Database connection failed: {os.getenv("DB_HOST")}, Amount: ¥{req.amount}')
    ...
    if resource_error_mode:
        ...
        infra_logger.error(f'[INFRA] Resource exhaustion: Memory allocation failed (100MB requested), Amount: ¥{req.amount}')
    ...
    if backend_only_failure_mode:
        ...
        infra_logger.error(f'[INFRA] Backend processing error: Internal error, Amount: ¥{req.amount}, Store: {req.storeId}')
    ...
    if failure_mode:
        try:
            ...
            infra_logger.error(f'[INFRA] External service timeout: Payment gateway (3s), Amount: ¥{req.amount}')
            newrelic.agent.add_custom_attributes([
